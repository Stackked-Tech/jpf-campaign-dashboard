...
import { InstrumentlLinkButton } from "@/components/grants/instrumentl-link-button";
...
export default async function InstrumentlDetailPage({
  ...
  const linkedId = (row.sf_opportunity_id as string | null) ?? null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <SourceBadge source={linkedId ? "linked" : "instrumentl"} />
        ...
        <InstrumentlLinkButton instrumentlId={id} currentSfId={linkedId} />
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
        <div className="divide-y divide-border">
          ...
          <Field
            ...
            value={
              row.internal_deadline
                ? formatDate(String(row.internal_deadline))
                : null
            }
          />
          <Field
          ...
          />
          <Field
            label="Next task / notes"
            value={row.next_task_and_notes}
            longText
          />
        </div>
      </div>
